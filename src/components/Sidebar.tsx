...
export default function Sidebar({ isCollapsed }: SidebarProps) {
  ...
  return (
    // Outer aside animates between w-60 and w-0.
    // overflow-hidden clips the inner content when collapsed.
    <aside
      className={`
        flex-shrink-0 bg-white border-r border-gray-200 flex flex-col h-screen
        transition-all duration-300 ease-in-out overflow-hidden
        ${isCollapsed ? 'w-0' : 'w-60'}
      `}
    >
      {/* Inner wrapper is always w-60 so content doesn't reflow during animation */}
